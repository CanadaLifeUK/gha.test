...
public class When_policy_contains_multiple_alive : BaseControllerTests
{
    [Fact]
    public async Task It_should_return_expected()
    {
        var policyNumber = "34567812";
        ...
        policyResponse.Clients.Count().Should().Be(3);
        ...
        policyResponse.Clients.Count(f => databasePolicies[2].ClientReference.ToString().Contains($"{f.ExternalReferenceNumber}")).Should().Be(1);
    }
}